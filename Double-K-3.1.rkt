...
(define deck
  ;; load 10 cards from the image dir
...
(define timebox-w 210)
(define timebox-h (+ 12 8 8))
(define timebox-x (/ (- cbw dx 25) 3))
...
(define (make-time-region secs)
  (make-region cbx timebox-y cbw timebox-h
               (if (>= secs 6000)
                 "--:--"
                 (format
                  "~a:~a"
